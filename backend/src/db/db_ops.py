...
def create_observation(dto: CreateObservationDto, session: Session) -> int:
...
def create_car_observation(dto: CreateCarObservationDto, session: Session) -> int:
...
def create_observation_sensor(dto: CreateObservationSensorDto, session: Session) -> str:
...
def create_generation(dto: CreateGenerationDto, session: Session) -> int:
...
def create_car(dto: CreateCarDto, session: Session) -> int:
...
def populate_generation_with_car_observations(
    dtos: list[CreateCarObservationDto], session: Session
) -> list[int]:
    for dto in dtos:
        if session.get(Car, dto.car_id) is None:
    ...
    all_observation_ids: set[int] = {id for dto in dtos for id in dto.observation_ids}
    observations = session.scalars(
    ...
    all_tpms_sensor_ids: set[str] = {
        observation_map[id].tpms_sensor_id for dto in dtos for id in dto.observation_ids
    }
    tpms_sensors = session.scalars(
        select(TPMSSensor).where(
...
def populate_generation_with_cars(
    dtos: list[CreateCarDto], session: Session
) -> list[int]:
    if len(dtos) == 0:
        return []
...
def append_observation_to_car_observation(
    car_observation_id: int, observation_id: int, session: Session
) -> None:
...
def update_car_name(car_id: int, new_name: str, session: Session) -> int:
...
def get_car(car_id: int, session: Session) -> CarResponseDto:
    car = session.get(Car, car_id)
    if car is None:
        raise ValueError(f"Car {car_id} does not exist")
    return car.to_dto()
...
def get_cars_for_tpms(tpms_id: str, session: Session) -> list[CarResponseDto]:
...
def get_cars_for_generation(
    generation_id: int, session: Session
) -> list[CarResponseDto]:
...
def get_car_observations_for_car(
    car_id: int, session: Session
...
def get_observation_sensor(
    observation_sensor_id: str, session: Session
) -> ObservationSensorResponseDto:
    observation_sensor = session.get(ObservationSensor, observation_sensor_id)
    if observation_sensor is None:
        raise ValueError(f"Observation sensor {observation_sensor_id} does not exist")
    return observation_sensor.to_dto()


def get_observations_for_car_observation(
    car_observation_id: int, session: Session
...
def get_generation(generation_id: int, session: Session) -> GenerationResponseDto:
    generation = session.get(Generation, generation_id)
    if generation is None:
        raise ValueError(f"Generation {generation_id} does not exist")
...
def get_all_tpms_sensors(session: Session) -> list[TPMSSensorResponseDto]:
    tpms_sensors = session.scalars(select(TPMSSensor)).all()
    return [tpms_sensor.to_dto() for tpms_sensor in tpms_sensors]


def get_all_observations(session: Session) -> list[ObservationResponseDto]:
    observations = session.scalars(select(Observation)).all()
    return [observation.to_dto() for observation in observations]


def get_all_observation_sensors(session: Session) -> list[ObservationSensorResponseDto]:
...
def get_all_generations(session: Session) -> list[GenerationResponseDto]:
...
def get_recent_car_observations_for_generation(
    generation_id: int, max_age_ms: int, session: Session
) -> list[CarObservationResponseDto]:
    if session.get(Generation, generation_id) is None:
        raise ValueError(f"Generation {generation_id} does not exist")
